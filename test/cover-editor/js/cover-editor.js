...
const COVER_TYPE = url.searchParams.get("coverType");

// const ORDERID = 123456789;
const ORDERID = url.searchParams.get("orderid");

// const DOC_WIDTH_MM = 306; // в мм
const DOC_WIDTH_MM = url.searchParams.get("width"); // в мм
const DOC_WIDTH_PX = fabric.util.parseUnit(DOC_WIDTH_MM + 'mm'); // в px
const DOC_WIDTH = DOC_WIDTH_PX * SCALE;

const DOC_HEIGHT_MM = url.searchParams.get("height"); // в мм
...
const DOC_SPINE_MM = url.searchParams.get("spine"); // в мм
...
const BOOK_NAME = url.searchParams.get("title");
const BOOK_AUTHOR = url.searchParams.get("author");

const SPINE_COLOR = '#b0b0b0';
const COVER_COLOR = '#e8e8e8';


let cover_spine_gap = 6;
let cover_bleed_mm = 17;
...
const COVER_BLEED_MM = cover_bleed_mm; // в мм
...
const COVER_SPINE_GAP_MM = cover_spine_gap; // в мм
...
buttonClearAll.onclick = function () {
  ...
  front.bgColor = COVER_COLOR;
  back.bgColor = COVER_COLOR;
  spine.bgColor = SPINE_COLOR;

  front.canvas.setBackgroundColor(front.bgColor, activeCanvas.canvas.renderAll.bind(activeCanvas.canvas));
  back.canvas.setBackgroundColor(back.bgColor, activeCanvas.canvas.renderAll.bind(activeCanvas.canvas));
  spine.canvas.setBackgroundColor(spine.bgColor, activeCanvas.canvas.renderAll.bind(activeCanvas.canvas));
  ...
  $.post("./cover-editor.php",
    ...
    function (data) {
      ...
      */
      ...
      /* 
      if (data == 'ok') {
        window.top.location.href = "/editus.php?do=orderstep4&orderid=" + ORDERID;
      }
      */
      ...
      form.setAttribute('action', `//editus.ru/test/editus.php?do=orderstep3&o=${ORDERID}`);
      form.setAttribute('method', 'POST');
      form.setAttribute('hidden', 'hidden');
      /*-----------*/
      var input1 = document.createElement('input');
      input1.setAttribute('type', 'hidden');
      input1.setAttribute('name', 'os2_newo');
