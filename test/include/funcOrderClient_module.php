...
function createcovertemplate($path,$size,$pages, $papertypeid, $covertype){
    ...
    $pagesinblock = $pages;
//    $iwkor = ($pagesinblock*$paperDensity/1600 + 1);
    if ($covertype == 'soft'){
	$iwkor = ($pagesinblock*$paperDensity/1600) + 1;
    } else {
	$iwkor = ($pagesinblock*$paperDensity/1600) + 4;
    }
//    $iwkor = ($pagesinblock*$paperDensity/1600 + 4 + отставы входят в шаблон);
...
//        $totalbind = ($pr_bind * $dataorder['orderCount'] * $koef);
        ...
        $paperTypeId = $dataorder['orderPaperBlock']; 
        ...
        if ($cover == 'soft') {
            $spine = (($page_count * $paper_density) / 1600) + 1;
            $calculated_width = ($width_block * 2) + $spine + 3 * 2;
            $calculated_height = $height_block + 3 * 2;
        } else {
            $spine = (($page_count * $paper_density) / 1600) + 4;
            $calculated_width = ($width_block * 2) + (6 * 2) + $spine + (17 * 2);
            $calculated_height = $height_block + 7 + 17 * 2;
        }

        $cover_width = ceil($calculated_width);
        $cover_height = ceil($calculated_height);
        
        $title = $_POST['os1_name'];
        $author = $_POST['os1_author'];

        $userId = $_SESSION['userId'];

        $editorUrl = "//editus.ru/test/cover-editor/cover-editor.html?width=$cover_width&height=$cover_height&spine=$spine&orderid=$orderid&title=$title&author=$author&userid=$userId&page_count=$page_count&paper_density=$paper_density&paperTypeId=$paperTypeId&coverType=$cover";
        ...
        $tpl->set_vars(array(
            'hrefcover'=>$hrefcover,
            'hrefcoverpdf'=>$hrefcoverpdf,
            'ftemplate'=>$ftemplate,
            'hrefcovertemplate'=>$hrefcovertemplate,
            'totalbind'=>$totalbind, 
            'totalcover'=>$totalcover,
            'orderid' => intval($_GET['o']), 
            'userid' => $_SESSION['userId'], 
            'action' => Main_config::$main_file_name . '?do=orderstep3&amp;o=' . intval($_GET['o']),
            'editorUrl' => $editorUrl,
        ));
