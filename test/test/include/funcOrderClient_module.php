...
function orderstep1(&$e, &$t)
{
        ...
        $db->query("SELECT max(orderId) FROM UsersOrders");
        $orderid = $db->fetch_array()[0];

        $t->addjs('__step1');
        $t->set_path_js('');
        $t->addjs('//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min');
        $t->__addcss('__styles');
        // $e->do ='listprojects';
        $tpl = new Template();
        $tpl->set_path('templates/');
...
function orderstep2(&$e, &$t)
{
    if (
        isset($_POST['book-name']) && isset($_POST['book-author']) &&
        isset($_POST['page-count']) && isset($_POST['book-size'])
    ) {
        $db = new Db();
        $db->query("SELECT max(orderId) FROM UsersOrders");
        $orderid = $db->fetch_array()[0];
        ...
        $db->query("INSERT INTO UsersOrders
                    SET orderId = '" . $orderid . "',
                        orderName = '" . $db->mres($_POST['book-name']) . "',
                        orderAutor = '" . $db->mres($_POST['book-author']) . "',
                        orderCount = 1,
                        orderPages = '" . $_POST['page-count'] . "',
                        orderSize = '" . $_POST['book-size'] . "',
                        formatUplBlock = 'pdf',
                        userId = '" . $_SESSION['userId'] . "',
                        orderstep = '1'");
        $db->query("INSERT INTO OrderStateChanges 
        ...
        // Перенести блок из временной папки
        $path_old = __DIR__ . "/../uploads/temp/".$_SESSION['userId']."/temp_block.pdf";
        $path_dir = __DIR__ . "/../uploads/".$_SESSION['userId']."/$orderid";
        if (!file_exists($path_dir)){
            mkdir($path_dir);
        }
        $path_new = $path_dir."/$orderid"."_block.pdf";
        rename($path_old, $path_new);


        $db->query("SELECT formatName, formatWidth, formatHeight
                    FROM PaperFormat
                    WHERE formatId = '" . $_POST['book-size'] . "' ");
        $row = $db->fetch_array();
        $formatName = $row['formatName'];
        $formatSizeText = $row['formatWidth'] . "x" . $row['formatHeight'];
        ...
        $tpl->set_vars(array(
            ...
            'format_size_text' => $formatSizeText,
            'pages' => $_POST['page-count'],
        ));
        $tpl->fetch('orderstep2.tpl');
        return $tpl->get_tpl();

    } else if ($_POST['do'] == 'getregion') {
        $db = new Db();
        ...
        foreach ($regions as $cur) {
            ...
            $res .= "<option $c value=".$cur['RegionId'].">".$cur['RegionName']."</option>";
        }

        echo $res;
        exit();
    } else if ($_POST['do']=='savenewaddress'){
        $db = new Db();
        ...
        foreach ($addreses as $cur) {
            ...
            $str = $cur['addressIndex'] . ', ' . $cur['addressCity'] . ', ' . $cur['addressStr'] . ', ' . $cur['addressHouse'] . ', ' . $cur['addressApt'];
            $res .= "<option $sell value=".$cur['addressId'].">".$str."</option>";
            $sell = '';
        }
        $res .= "<option value=\"new\">"._OS3_ADDADDRESE."</option>";

        echo $res;
        exit();
    } else if ($_POST['do'] == 'getproviders') {
        $db = new Db();
        ...
        if ($_POST['massa'] < 1000) {
            $db->query("SELECT DeliveryProviders.DeliveryProviderName AS DeliveryProviderName, 
                    ...
                    WHERE UsersDeliveryAddreses.addressId = '" . intval($_POST['addressid']) . "' AND
                            ...
                            DeliveryProvidersCosts.minWeight < '" . $_POST['massa'] . "' AND
                            DeliveryProvidersCosts.maxWeight > '" . $_POST['massa'] . "' ");
            while ($row = $db->fetch_array()) {
                $providers[] = $row;
            }
        } else {
            $db->query("SELECT DeliveryProviders.DeliveryProviderName AS DeliveryProviderName, 
                    ...
                    WHERE UsersDeliveryAddreses.addressId = '" . intval($_POST['addressid']) . "' AND
                            ...
                            DeliveryProvidersCosts.maxWeight > '" . $_POST['massa'] . "' ");
            while ($row = $db->fetch_array()) {
                $row['DeliveryProvidersCosts'] = $row['DeliveryProvidersCosts'] + $row['OverQuote'] * ceil(($_POST['massa'] / 1000) - 1);
                $providers[] = $row;
            }
        }
        ...
        if (count($providers) > 0) {
            $fis = 'selected="selected"';
            $massa_print = round($_POST['massa'] / 1000, 1) . " кг ";

            foreach ($providers as $cur) {
                $delname = $cur['DeliveryProviderName'];
                $delcost = $cur['DeliveryProvidersCosts'];
                $delcostid = $cur['DeliveryProvidersCostsId'];
                $res .= "<option $fis title=\"$delcost\"  value=\"$delcostid\">
                            $delname ( $massa_print - $delcost  руб.)
                        </option>";
                $fis = '';
            }
        } else {
            $res = '<option value="">Нет доступных вариантов</option>';
        }
    ...
    // if (!empty($_POST['size']) && !empty($_POST['volume']) && !empty($_POST['colorblock'])){
    // $data['cover'] = $_POST['cover'];
    // $data['paper'] = intval($_POST['paperblock']);
    // $data['size'] = intval($_POST['size']);
    // $data['binding'] = intval($_POST['binding']);
...
{
    // Почитать про изм таблицы (ламинация новый столбец)
    // Посмотреть как добавить доставку в usersorders
    if (
        isset($_POST['colorblock']) && isset($_POST['paperblock']) &&
        isset($_POST['binding']) && isset($_POST['lamination']) &&
        isset($_POST['typedeliv']) && isset($_POST['orderid']) &&
        isset($_POST['count']) && isset($_POST['orderid'])
    ) {

        $db = new Db();

        $db->query("SELECT CoverType FROM BindingType WHERE BindingId = '" . $_POST['binding'] . "'");
        $cover = $db->fetch_array()[0];

        if ($_POST['additionalservice']!= NULL){
            $oad = implode(',', $_POST['additionalservice']);
        }else{
            $oad = '';
        }

        $db->query("UPDATE UsersOrders
                    SET orderColorBlock = '" . $_POST['colorblock'] . "',
                        orderPaperBlock = '" . $_POST['paperblock'] . "',
                        orderBinding = '" . $_POST['binding'] . "',
                        orderCover = '" . $cover . "',                       
                        orderCount = '" . $_POST['count'] . "',
                        orderAdditService = '" . $oad . "',                       
                        orderstep = '2'
                        WHERE orderId = '" . $_POST['orderid'] . "'");

        // Проверка соотв-я размеру обложки
        $db->query("SELECT orderSize, orderPages
                    FROM UsersOrders
                    WHERE orderid = '" . $_POST['orderid'] . "' ");
        $row = $db->fetch_array();
        $formatId = $row['orderSize'];
        $page_count = $row['orderPages'];
        $paperTypeId = $_POST['paperblock'];
        ...
        $db->query("SELECT orderName, orderAutor
                    FROM UsersOrders
                    WHERE orderid = '" . $_POST['orderid'] . "' ");
        $row = $db->fetch_array();

        $author = $row['orderAutor'];
        $title = $row['orderName'];
        ...
        $tpl->set_vars(array(
            ...
            'cover' => $cover,
            'cover_width' => $calculated_width,
            'cover_height' => $calculated_height,
            'spine' => $spine_thickness,
            'author' => $author,
            'title' => $title,
        ));
...
function vieworder(&$e)
{
    if (is_numeric($_GET['o'])) {
        ...
        $db->query("SELECT PaperFormat.formatName AS formatName,
                           PaperTypeCostsBlock.PaperTypeName AS PaperTypeName,
                           PaperTypeCostsBlock.PaperTypeWeight AS PaperTypeWeight,
                           BindingType.BindingName AS BindingName,
                           PrintTypeCostsBlock.PrintTypeName AS PrintTypeName 
...
function orderstep4(&$e, &$t, &$u)
{
    ...
    $path_dir = "/test/uploads/".$_SESSION['userId']."/$orderid";
    $block_path = $path_dir."/$orderid"."_block.pdf";
    $cover_path = $path_dir."/$orderid"."_cover.pdf";

    // $pdf_path = '../../book.pdf';

    $t->__addcss('pdf-viewer');
    // $t->addjs('jquery-1.4.2.min');
    $t->addjs('__step4');
    $t->set_path_js('');
    // $t->addjs('https://unpkg.com/pdfjs-dist@2.0.489/build/pdf.min');
    // $t->addjs('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.3.0/jspdf.umd.min');
    // $t->addjs('https://code.jquery.com/jquery-migrate-3.3.2');
    // В самый конец
    $t->set_path_js('js/');
    // $t->addjs('showpdf');
    ...
    $tpl = new Template();
    // $tpl->addjs('jquery.simplemodal.1.4.1.min');
    $tpl->set_path('templates/');
    $tpl->set_vars(array('block_path' => $block_path, 'cover_path' => $cover_path));
    $tpl->fetch('orderstep4.tpl');
    return $tpl->get_tpl();
}

function orderstep4_old(&$e, &$t, &$u)
{
    if (!isset($_GET['a'])) {
        if (is_numeric($_GET['o'])) {
            if (isset($_POST['os3_newo'])) {
