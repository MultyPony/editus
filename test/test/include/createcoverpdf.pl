...
if ($ARGV[0] =~ /\.jpg$/)
	...
	`magick convert -density 300 $srcfname -background white -flatten -alpha off $jpgfname`;
	}
$pdffname=$srcfname;
...
if ($isbn) 
	...
	`C:/Python/Python38/python.exe $dirname/createbarcodepdf.py $bcfname $isbn`;
	}
else
	{ 
	# `/usr/bin/python $dirname/createbarcodepdf.py $bcfname $&`;
	`C:/Python/Python38/python.exe $dirname/createbarcodepdf.py $bcfname $&`;
	}
# my $params = `/usr/bin/identify -format "%w %h" $jpgfname`;
my $params = `magick identify -format "%w %h" $jpgfname`;
my ($w,$h) = split(' ',$params);
...
`C:/Python/Python38/python.exe $dirname/createcoverpdf_boxes.py $pdfwbox $outname`;
