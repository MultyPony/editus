...
$(document).ready(function () {
    ...
    //    мягкая обложка: min = 40 (было 20), max = 560
    ...
    /*
     ...
     Толщина корешка = (С*Б/1600)*1,1 = новая формула (С*Б/1600)+1
    ...
    function calculateSpine() {
			...
			var spine = Number(((pages * density / 1600) + 1).toFixed(0));
    ...
    function resizePreview() {
        ...
        if (cover == 'calcSoftCover') {
            ...
            //Размеры макета обложки (width х height) ) – width = ширина х 2 + отставы 6мм + разница с мягкой 3мм + толщина корешка, height = высота + 10.
            var modelH = Math.round(h.slice(0, -2) * 1 + 7);
            var modelW = Math.round(w.slice(0, -2) * 2 + 15 + spine);
            $("label[for = calcCoverModelBleeds]").text("Загибы со всех сторон (bleeds):");
            $("#calcCoverModelBleeds").val(plus + 17 + dimensionUnit);
