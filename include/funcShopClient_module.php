       ...
       if (isset($_POST['so1_newo'])){
            ...
            if ($db->num_rows() > 0) {
				...
				if ($_POST['typedeliv']=='pickup-point') {
                    $delivprovid = 18;
                    $delivcost = intval($_POST['so1_delivery_price']);
                } else {
                ...
                if ($_POST['typedeliv']=='pickup-point') {
                    $db->query("INSERT INTO UsersDeliveryAddreses 
                                SET userId = '" . $_SESSION['userId'] . "',
                                    addressIndex = '" . $orderId . "',
                                    addressStr = '" . $db->mres($_POST['so1_pvz_address']) . "'");
                    $db->query("SELECT LAST_INSERT_ID()");
                    $pickUpAddressId = $db->fetch_array();
                }
    ...
    if (is_numeric($_GET['o'])){
        ...
        if ($data['DeliveryProviderId']!=0){
            ...
            if ($data['DeliveryProviderId']==18) {
                ...
                $db->query("SELECT addressStr, addressIndex
                        FROM UsersDeliveryAddreses
