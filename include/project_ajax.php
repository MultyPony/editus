...
<p><a class="more-link" href="//editus.ru/new/online.html#paper" target="_blank">Какую бумагу лучше выбрать?</a></p>
            <h3>Дополнительно</h3>
    ...
    if ($_POST['do'] == 'getPaperTypeBind'){
        ...
        if ($db->num_rows() >= 1){
            ...
            <p><a class="more-link" href="//editus.ru/new/pereplet.html" target="_blank">Какой переплет выбрать?</a></p><?php
        }else{
            ?>
...
<br><br>
    ...
    if ($_POST['do'] == 'getPaperTypeBindForCalulator'){
        $db = new Db();
        
        $db->query("SELECT BindingType.BindingId, BindingType.BindingName, CoverType 
                    FROM BindingType WHERE BindingType.BindingMin <= '".intval($_POST['pages'])."' 
